...
export const tableCalculateReducer = (
  state: TableCalculateState = {
    ...
    prepayPercent: 0,
    rows: [],
    precentBankBefore: 0.0729,
    precentBankAfter: 0.1065,
    discountMonths: 12,
  },
  action: BaseAction
) => {
    ...
    case "CALCULATE_COMPLETED":
      ...
      if (!data.carPrice) {
        ...
        if (
          ...
          action.payload["borrowingPrice"] ||
          action.payload["carPrice"]
        ) {
      ...
      for (let i = 1; i <= data.month; i++) {
        ...
        let row = createData(
          String(i),
          roundNumber(data_row.recurring_number),
          roundNumber(data_row.interest),
          roundNumber(data_row.origin),
          data_row.debt
        );
        data.rows.push(row);
      }
      data.rows.push(
        createData(
          "TOTAL",
          roundNumber(total_recurring_number),
          roundNumber(total_interest),
          roundNumber(data.rows[0]["debt"]),
          null
        )
      );
      return data;
